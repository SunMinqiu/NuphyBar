...
NuphyBar 是一个轻量的原生 macOS 菜单栏应用。它接收 Codex、Claude Code、Antigravity、OpenCode 等 Agent 的生命周期事件，并按照当前键盘协议发送简短的 HID 报告。NuPhy 定制固件在键盘本地渲染动画；AULA F99 Pro 使用原厂实时 RGB 模式显示整键纯色。
...
**AULA F99 Pro** 的蓝牙路径使用原厂非持久化 `0x88` 实时 RGB 命令，改变整键背光且不写入键盘配置闪存。原厂实时模式会自动超时，因此连接这款键盘时，NuphyBar 每秒补发一次当前颜色。空闲和完成均为绿色常亮，工作、工具执行和错误为红色，输出为黄色，等待为蓝色。右侧独立灯条不参与状态显示，因为它的 HID 配置路径会持久写入闪存。
...
| NuphyBar | 聚合多个会话，并按照当前键盘协议发送显示状态 | 不连续发送动画帧 |
| 键盘 | 渲染 NuPhy 本地动画或 AULA 整键纯色 | 不读取 Agent 内容 |

NuPhy 键盘只在显示状态改变时收到一份报告。AULA F99 Pro 的原厂实时模式会超时，因此每秒收到一次当前纯色保活。NuphyBar 不会连续传输“第一颗灯亮、第二颗灯亮……”这样的动画帧。
...
多个 Agent 同时运行时，一个会话完成不会错误地盖住另一个仍在工作的会话。完成状态保留约 15 秒。活动状态不再自动过期，只会由明确的生命周期事件结束。
